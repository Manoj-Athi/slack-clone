  ...
  return (
    <div className={` md:translate-x-0 md:static z-10 translate-x-[${slideIn}] absolute flex h-[calc(100vh_-_58px)] w-[14rem] flex-col space-y-2 border-r-2 border-gray-200 bg-[#4a154be6] text-white p-4 transition`}>
        ...
        { dropdownChannel && (
          <div>
            { workSpace.channels.filter((channel) => channel.isGroupChannel === true).map((channel) => (
              <p onClick={() => setCurrentChat(channel)} key={channel._id} className='py-1 cursor-pointer'># { channel.channelName }</p>
            )) }
          </div> 
        )}
        <div>
          <button className="text-slate-300 text-sm cursor-pointer" onClick={handleModal}>Add Channels</button> 
        </div>
          {/* {
            popupChannel &&  (
      ...
      <section className='py-2'>
        ...
        <div>
        <button className="text-slate-300 text-sm cursor-pointer" onClick={handleDirectModal}>Add Direct Messages</button> 
          {/* {
            popupDirect &&  (
            <div>
